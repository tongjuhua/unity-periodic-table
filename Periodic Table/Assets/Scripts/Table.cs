...
{
    [SerializeField] private GameObject _tableObj;
    ...
    void Start()
    {
        ...
        {
            var index = Convert.ToInt32(_data[i][0]);
            var textMesh = transform.GetChild(index - 1).GetChild(0).GetComponent<TextMesh>();
            textMesh.text = _data[i][0] + Environment.NewLine
                            + _data[i][1] + Environment.NewLine
                            + _data[i][2] + Environment.NewLine
                            + _data[i][3] + Environment.NewLine;
            textMesh.transform.localPosition = new Vector3(0, 0.4f, -0.6f);
